...
<?php
    ...
    function sendUser () {
        ...
        $dbsTestName = 'SELECT count(id) AS countUser,
                        ...
                        FROM '.USERS_TABLE.'
                        WHERE pseudo = "'.$_CLEAN['pseudo'].'" ';
        $dbeTestName = mysql_query($dbsTestName) or die(nk_debug_bt());
            ...
            */
            ...
            $dbiUserInsert  = ' INSERT INTO '.USERS_TABLE.'
                                ...
                                VALUES          (
                                                    ...
                                                    "'.$cryptPass.'" , "'.time().'" , "'.$_CLEAN['avatar'].'" , "'.$_CLEAN['signature'].'" ,
                                                    "" , "" , "" , "'.$_CLEAN['country'].'" , "" ,
                                                    "" , "" , "" , "1" , "1"
                                                )';
            $dbeUserInsert  = mysql_query($dbiUserInsert) or die (nk_debug_bt());
            ...
            $dbeUserDetail  = mysql_query($dbiUserDetail) or die (nk_debug_bt());

            $actionMsg = 'ajout avec succes le ';

            $texteaction = $actionMsg.': '.$_CLEAN['pseudo'];
            mysql_query('INSERT INTO '.ACTIONS_TABLE.' VALUES ("", "'.time().'", "'.$GLOBALS['user']['id'].'", "'.$texteaction.'", "") ') or die (nk_debug_bt());
